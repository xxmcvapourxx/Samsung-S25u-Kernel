...
exif_entry_initialize (ExifEntry *e, ExifTag tag)
{
	...
	case EXIF_TAG_OFFSET_TIME_DIGITIZED:
	{
		...
		e->data = exif_entry_alloc (e, e->size);
		if (!e->data) break;
		// In the exif spec 2.3
		// when the offsete are unknown, all the character spaces except colons (":")
		// should be filled with blank characters [20.H].
		// The character string length is 7 Bytes including NULL for termination.
		snprintf ((char *) e->data, e->size,
			"%c%c%c:%c%c%c",
