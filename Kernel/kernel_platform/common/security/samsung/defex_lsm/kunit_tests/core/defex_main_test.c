...
static void task_defex_zero_creds_test(struct kunit *test)
{
	...
	mock_task = kthread_run(kunit_mock_thread_function, NULL,
		"task_defex_zero_creds_test_thread");
...
static void task_defex_safeplace_test(struct kunit *test)
{
	...
	mock_task = kthread_run(kunit_mock_thread_function, NULL,
		"task_defex_safeplace_test_thread");
	get_task_struct(mock_task);
	KUNIT_ASSERT_FALSE(test, IS_ERR_OR_NULL(mock_task));
	ssleep(1);
...
static void task_defex_immutable_test(struct kunit *test)
{
	...
	KUNIT_EXPECT_EQ(test, task_defex_immutable(&dc, feature_immutable_path_open),
		DEFEX_ALLOW);

	/* Deny case - !feature_immutable_path_open */
	KUNIT_EXPECT_EQ(test, task_defex_immutable(&dc, feature_immutable_path_write),
		-DEFEX_DENY);
...
static void task_defex_enforce_test(struct kunit *test)
{
	...
	mock_task = kthread_run(kunit_mock_thread_function, NULL,
		"task_defex_enforce_test_thread");
	get_task_struct(mock_task);
	KUNIT_ASSERT_FALSE(test, IS_ERR_OR_NULL(mock_task));
	ssleep(1);
	...
	/* If task_defex_safeplace == -DEFEX_DENY and FEATURE_SAFEPLACE_SOFT disabled,
	 * kill process
	 */
	...
	if (!(defex_get_features() & FEATURE_SAFEPLACE_SOFT)) {
		KUNIT_EXPECT_EQ(test, task_defex_enforce(mock_task, test_file, __DEFEX_execve),
			-DEFEX_DENY);
		put_task_struct(mock_task);
	} else {
		KUNIT_EXPECT_EQ(test, task_defex_enforce(mock_task, test_file, __DEFEX_execve),
	...
	if (defex_get_features() & FEATURE_IMMUTABLE_SOFT)
		KUNIT_EXPECT_EQ(test, task_defex_enforce(mock_task, test_file, __DEFEX_write),
			DEFEX_ALLOW);
	else
		KUNIT_EXPECT_EQ(test, task_defex_enforce(mock_task, test_file, __DEFEX_write),
			-DEFEX_DENY);
...
static void task_defex_check_creds_test(struct kunit *test)
{
	...
	/* Creates a mock task */
	mock_task = kthread_run(kunit_mock_thread_function, NULL,
		"task_defex_check_creds_test_thread");
	get_task_struct(mock_task);
	KUNIT_ASSERT_FALSE(test, IS_ERR_OR_NULL(mock_task));
	ssleep(1);
	init_defex_context(&dc, 0, mock_task, NULL);
	...
	/* mock_task->uid == 0 && (mock_task->tgid !=
	 *	mock_task->pid && mock_task->tgid != 1) --> -DEFEX_DENY
	 */
	backup_pid = mock_task->pid;
...
static void kill_process_test(struct kunit *test)
{
	...
	KUNIT_EXPECT_EQ(test, kill_process(mock_task), 0L);
#else
	KUNIT_SUCCEED(test);
#endif
}
