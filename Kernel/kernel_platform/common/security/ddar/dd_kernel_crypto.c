...
int dd_generate_file_key(char userId, struct dd_crypt_context *crypt_context)
{
	...
	memcpy(crypt_context->cipher_file_encryption_key,
			cipher_file_encryption_key, mode->keysize);
...
int get_dd_master_key(int userid, void *key)
{
	...
	WARN_ON_ONCE(dd_master_key->mode != FS_ENCRYPTION_MODE_AES_256_GCM);
	WARN_ON_ONCE(dd_master_key->size != DD_AES_256_GCM_KEY_SIZE);
...
int dd_dump_key(int userid, int fd)
{
	...
	if (dd_policy_kernel_crypto(crypt_context.policy.flags) && S_ISREG(inode->i_mode)) {
		...
		if (rc) {
			dd_error("[DUALDAR_DUMP] failed to retrieve dd master key, rc:%d\n", rc);
			rc = -ENOKEY;
			goto out;
		}
		dd_hex_key_dump("[DUALDAR_DUMP] INNER LAYER MASTER KEY",
				dd_inner_master_key.raw, dd_inner_master_key.size);

		rc = get_dd_file_key(&crypt_context,
				&dd_inner_master_key, dd_inner_file_encryption_key);
		if (rc) {
			dd_error("[DUALDAR_DUMP] failed to retrieve inner fek, rc:%d\n", rc);
			goto out;
		}
		dd_hex_key_dump("[DUALDAR_DUMP] INNER LAYER FILE ENCRYPTION KEY",
				dd_inner_file_encryption_key, mode->keysize);

	} else {
		dd_error("[DUALDAR_DUMP] DUMP KEYS FOR THIRD-PARTY CRYPTO\n");
		dd_error("[DUALDAR_DUMP] INNER LAYER MASTER KEY : SKIP\n");
		dd_error("[DUALDAR_DUMP] INNER LAYER FILE ENCRYPTION KEY : SKIP\n");
	}
	...
	dd_hex_key_dump("[DUALDAR_DUMP] OUTER LAYER MASTER KEY",
			dd_outer_master_key.raw, dd_outer_master_key.size);
	...
	dd_hex_key_dump("[DUALDAR_DUMP] OUTER LAYER FILE ENCRYPTION KEY",
			dd_outer_file_encryption_key.raw, dd_outer_file_encryption_key.size);
		...
		struct fscrypt_key *dd_master_key, unsigned char *raw_key)
{
	...
	dd_dump("ddar file key (cipher)", crypt_context->cipher_file_encryption_key, mode->keysize);
	memcpy(cipher_file_encryption_key,
			crypt_context->cipher_file_encryption_key, mode->keysize);
	...
	aead_request_set_ad(aead_req, strlen(aad));
	aead_request_set_crypt(aead_req, src_sg, dst_sg,
			mode->keysize + DD_AES_256_GCM_TAG_SIZE, iv);
		...
		struct bio *clone, enum dd_crypto_direction_t rw)
{
	...
	WARN_ON_ONCE(rw == DD_ENCRYPT && !clone);
	...
	while (orig->bi_iter.bi_size) {
		if (rw == DD_ENCRYPT) {
			...
			WARN_ON_ONCE(info->ino != orig_bv.bv_page->mapping->host->i_ino);
			dd_dump("dd_crypto_bio_pages::encryption start",
					page_address(plaintext_page), PAGE_SIZE);
			rc = dd_sec_crypt_page(info, DD_ENCRYPT, plaintext_page, ciphertext_page);
			if (rc) {
				dd_error("failed encryption rc:%d\n", rc);
				return rc;
			}
			dd_dump("dd_crypto_bio_pages::encryption finished",
					page_address(ciphertext_page), PAGE_SIZE);
			...
			WARN_ON_ONCE(info->ino != orig_bv.bv_page->mapping->host->i_ino);
			dd_dump("dd_crypto_bio_pages::decryption start",
					page_address(ciphertext_page), PAGE_SIZE);
			...
			dd_dump("dd_crypto_bio_pages::decryption finished",
					page_address(ciphertext_page), PAGE_SIZE);
