...
static inline int __get_xattr(struct inode *inode,
	...
	if (inode->i_sb->s_cop->android_oem_data1[0])
		return ((int (*)(struct inode*, const char*, void*, size_t))(
				inode->i_sb->s_cop->android_oem_data1[0]))(
...
static inline int __set_xattr(struct inode *inode,
	...
	if (inode->i_sb->s_cop->android_oem_data1[1])
		return ((int (*)(struct inode*, const char*, const void*, size_t, void *))(
...
int dd_write_crypt_context(struct inode *inode,
		const struct dd_crypt_context *context, void *fs_data)
{
	return __set_xattr(inode, XATTR_NAME_DD_POLICY, context,
			sizeof(struct dd_crypt_context), fs_data);
}
