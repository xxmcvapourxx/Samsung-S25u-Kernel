				  ...
				  struct waltgov_cpu *wg_cpu, u64 time)
{
	...
	unsigned int debug_smart_freq = 0;
	...
	if (freq > smart_freq) {
		freq = debug_smart_freq = smart_freq;
...
out:
	...
	if (enable_logging) {
		char trace_smart_freq[32] = {0, };
		snprintf(trace_smart_freq, sizeof(trace_smart_freq), "smart_freq_%d", policy->cpu);
		trace_clock_set_rate(trace_smart_freq, debug_smart_freq, raw_smp_processor_id());
	}
