...
{
	struct f_ss_acm    *acm = (struct f_ss_acm *)dev;
...
DECLARE_USB_FUNCTION(ss_acm, ss_acm_alloc_instance, ss_acm_alloc_func);
static int __init ss_acm_init(void)
{
#ifdef CONFIG_USB_DUN_SUPPORT
	int err;
...
#endif
	return usb_function_register(&ss_acmusb_func);
}
static void __exit ss_acm_exit(void)
{
	return usb_function_unregister(&ss_acmusb_func);
}
module_init(ss_acm_init);
module_exit(ss_acm_exit);
MODULE_LICENSE("GPL");
