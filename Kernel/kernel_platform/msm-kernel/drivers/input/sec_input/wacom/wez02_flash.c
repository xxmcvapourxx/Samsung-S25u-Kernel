...
static bool flash_erase_all(struct wacom_data *wacom)
{
	...
	bRet = wacom_set_feature(wacom, REPORT_ID_1, len, command, COMM_REG, DATA_REG);
	if (!bRet) {
		input_err(true, wacom->dev,
				"%s failed to set feature\n", __func__);
		return false;
...
static bool flash_erase_reserved(struct wacom_data *wacom)
{
	bool bRet = false;
	...
	for (i = 48; i < 64; i++) {
		...
		command[len++] = i;				/* Report:erased block No. */
		...
		if (!bRet) {
			input_err(true, wacom->dev,
					"%s failed to set feature(REPORT_ID_1): %d\n", __func__, i);
			return bRet;
		}
		...
		if (ECH_len == 3) {
			for (j = 0; j < 3; j++) {
				...
				do {
					bool bRet = false;
					bRet = wacom_get_feature(wacom, REPORT_ID_2, BOOT_RSP_SIZE, response, COMM_REG, DATA_REG, 0);
					if (!bRet) {
						input_err(true, wacom->dev,
								"%s failed to set feature(REPORT_ID_2): %d\n", __func__, i);
						return bRet;
					}

					if ((ret = check_progress(wacom, &response[1], (BOOT_RSP_SIZE - 3), BOOT_ERASE_FLASH, ECH_ARRAY[j])) < 0) {
						input_err(true, wacom->dev,
								"%s failed to check_progress: %d\n", __func__, i);
						return false;
					}
				} while (ret == PROCESS_INPROGRESS);
			}
	...
	if(ECH_len != 0) {
		for (j = 0; j < ECH_len; j++) {
			...
			do {
				...
				if (!bRet) {
					input_err(true, wacom->dev,
							"%s failed to get feature(ECH_len, REPORT_ID_2): %d\n", __func__, j);
					return bRet;
				}

				if ((ret = check_progress(wacom, &response[1], (BOOT_RSP_SIZE - 3), BOOT_ERASE_FLASH, ECH_ARRAY[j])) < 0) {
					input_err(true, wacom->dev,
							"%s failed to check_progress, ECH_len: %d\n", __func__, j);
					return false;
				}
			} while (ret == PROCESS_INPROGRESS);
		}
		...
		for (j = 0; j < ECH_len; j++) {
			do {
				...
				if (!bRet) {
					input_err(true,
							wacom->dev,
							"%s failed to set feature: %d\n",
							__func__, j);
					return bRet;
				}

				if ((ret = check_progress(wacom, &response[1], (BOOT_RSP_SIZE - 3), 0x01, ECH_ARRAY[j])) < 0) {
					input_err(true,
							wacom->dev,
							"%s failed to check_progress: %d\n",
							__func__, j);
...
static int wacom_flash_w9020(struct wacom_data *wacom, unsigned char *fw_data)
{
	...
	input_info(true, wacom->dev,
			"%s erasing the reserved firmware\n", __func__);
	bRet = flash_erase_reserved(wacom);
	if (!bRet) {
		input_err(true, wacom->dev,
				"%s failed to erase the reserved firmware\n", __func__);
		return -EXIT_FAIL_ERASE;
	}
