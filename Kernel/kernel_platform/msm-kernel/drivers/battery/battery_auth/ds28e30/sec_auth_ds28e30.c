 ...
 *	2. sales_code != TUR
 */
		if (slavepcb != eur_slavepcb && !is_sales_code("TUR")) {
...
static int check_authic_supported(struct device_node *np)
{
	...
	fac_slavepcb_check = of_property_read_bool(np, "samsung,fac_slavepcb_check");
	if (fac_slavepcb_check) {
		ret = eur_slave_detection(np);
		if (ret < 0)
			return ret;
		goto AUTH_IC_DEVICE;
	} else {
		pr_err("%s: 'samsung,fac_slavepcb_check' not found\n", __func__);
	}
