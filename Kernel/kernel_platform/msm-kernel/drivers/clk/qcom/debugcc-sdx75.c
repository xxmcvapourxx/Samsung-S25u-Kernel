...
static struct clk_hw *debugcc_sdx75_hws[] = {
...
static int clk_debug_sdx75_probe(struct platform_device *pdev)
{
	struct clk *clk;
	int ret = 0, i;
	...
	for (i = 0; i < ARRAY_SIZE(debugcc_sdx75_hws); i++) {
		clk = devm_clk_register(&pdev->dev, debugcc_sdx75_hws[i]);
		if (IS_ERR(clk)) {
			dev_err(&pdev->dev, "Unable to register %s, err:(%ld)\n",
				clk_hw_get_name(debugcc_sdx75_hws[i]),
				PTR_ERR(clk));
			return PTR_ERR(clk);
		}
	}
...
static int __init clk_debug_sdx75_init(void)
{
	return platform_driver_register(&clk_debug_driver);
}
fs_initcall(clk_debug_sdx75_init);

MODULE_DESCRIPTION("QTI DEBUG CC SDX75 Driver");
MODULE_LICENSE("GPL");
