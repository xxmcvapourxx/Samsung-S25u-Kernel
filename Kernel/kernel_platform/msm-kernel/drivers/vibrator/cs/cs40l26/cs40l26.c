...
int cs40l26_pm_state_transition(struct cs40l26_private *cs40l26, enum cs40l26_pm_state state)
{
	...
	case CS40L26_PM_STATE_WAKEUP:
		...
		if (error) {
			dev_err(dev, "Failed to issue WAKEUP command\n");
			return error;
		}
	...
	case CS40L26_PM_STATE_PREVENT_HIBERNATE:
		for (i = 0; i < CS40L26_DSP_STATE_ATTEMPTS; i++) {
			error = cs40l26_mailbox_write(cs40l26, cmd);
			if (error) {
				dev_err(dev, "Failed to issue PREVENT HIBERNATE command\n");
				return error;
			}

			error = cs40l26_dsp_state_get(cs40l26, &curr_state);
			if (error) {
				dev_err(dev, "Failed to get DSP state\n");
				return error;
			}
			...
			if (curr_state == CS40L26_DSP_STATE_STANDBY) {
				...
				if (error) {
					dev_err(dev, "Failed to check PM lock\n");
					return error;
				}
		...
		if (cs40l26->allow_hibernate_sent) {
			...
			error = cs40l26_dsp_write(cs40l26,
						...
						he_time_cmd);
			if (error) {
				dev_err(dev, "Failed to issue HE time command\n");
				return error;
			}
		}
	...
	case CS40L26_PM_STATE_ALLOW_HIBERNATE:
		...
		if (error) {
			dev_err(dev, "Failed to issue ALLOW HIBERNATE command\n");
			return error;
		}
	...
	case CS40L26_PM_STATE_SHUTDOWN:
		...
		error = cs40l26_mailbox_write(cs40l26, cmd);
		if (error) {
			dev_err(dev, "Failed to issue SHUTDOWN command\n");
			return error;
		}
...
/* This function made by cirrus, samsung moved location for samsung_recovery() */
void cs40l26_irq_enable(struct cs40l26_private *cs40l26, const unsigned int en)
{
#ifdef CONFIG_CS40L26_SAMSUNG_FEATURE
	dev_info(cs40l26->dev, "%s en: %d irq_depth %d\n", __func__, en, cs40l26->irq_depth);
#endif
...
static int cs40l26_dsp_config(struct cs40l26_private *cs40l26)
{
	...
	if (error) {
#ifdef CONFIG_CS40L26_SAMSUNG_FEATURE
		dev_err(dev, "%s:Failed to wseq_init\n", __func__);
#endif
		return error;
	}
	...
	error = cs40l26_wseq_write(cs40l26, CS40L26_TST_DAC_MSM_CONFIG, CS40L26_SPK_DEFAULT_HIZ,
			true, CS40L26_WSEQ_OP_WRITE_H16, &pseq_params);
	if (error) {
		dev_err(dev, "Failed to set HI-Z in power on sequence\n");
		return error;
	}
	...
	if (cs40l26->revid < CS40L26_REVID_B2) {
		...
		if (error) {
			dev_err(dev, "Failed to handle A1 errata\n");
			return error;
		}
	}
	...
	error = cl_dsp_get_reg(cs40l26->dsp, "HALO_STATE", CL_DSP_XM_UNPACKED_TYPE, cs40l26->fw_id,
			&reg);
	if (error) {
		dev_err(dev, "Failed to find HALO_STATE register\n");
		return error;
	}
	...
	if (error) {
		dev_err(dev, "Failed to configure wakesources\n");
		return error;
	}
	...
	if (error) {
		dev_err(dev, "Failed to configure BST DCM\n");
		return error;
	}

	error = cs40l26_bst_ipk_config(cs40l26);
	if (error) {
		dev_err(dev, "Failed to configure BST IPK\n");
		return error;
	}

	error = cs40l26_bst_ctl_config(cs40l26);
	if (error) {
		dev_err(dev, "Failed to configure BST CTL\n");
		return error;
	}

	error = cs40l26_clip_lvl_config(cs40l26);
	if (error) {
		dev_err(dev, "Failed to configure clipping level\n");
		return error;
	}
	...
	if (!cs40l26->dbc_tuning_loaded) {
		...
		if (error) {
			dev_err(dev, "Failed to configure DBC\n");
			return error;
		}
	} else {
		error = cs40l26_dbc_enable(cs40l26);
		if (error) {
			dev_err(dev, "Failed to enable DBC\n");
			return error;
		}
	}
	...
	if (error) {
		dev_err(dev, "Failed to configure zero-cross\n");
		return error;
	}

	error = cs40l26_noise_gate_config(cs40l26);
	if (error) {
		dev_err(dev, "Failed to configure noise gate\n");
		return error;
	}

	error = cs40l26_aux_noise_gate_config(cs40l26);
	if (error) {
		dev_err(dev, "Failed to configure AUX noise gate\n");
		return error;
	}
	...
	if (!cs40l26->vibe_init_success) {
		...
		if (error) {
			dev_err(dev, "Failed to configure calibration DT\n");
			return error;
		}
	}
	...
	if (error) {
		dev_err(dev, "Failed to initialize brownout prevention\n");
		return error;
	}

	error = cs40l26_dc_wd_config(cs40l26);
	if (error) {
		dev_err(dev, "Failed to configure DC Watchdog\n");
		return error;
	}

	error = cs40l26_amp_drv_slope_config(cs40l26);
	if  (error) {
		dev_err(dev, "Failed to configure AMP DRV slope\n");
		return error;
	}
	...
	if (error) {
#ifdef CONFIG_CS40L26_SAMSUNG_FEATURE
		dev_err(dev, "Failed to PM Runtime Resume\n");
#endif
		return error;
	}
	...
	error = cl_dsp_get_reg(cs40l26->dsp, "TIMEOUT_MS",
			CL_DSP_XM_UNPACKED_TYPE, CS40L26_VIBEGEN_ALGO_ID, &reg);
	if (error) {
		dev_err(dev, "Failed to find TIMEOUT_MS register\n");
		goto pm_err;
	}
	...
	if (error) {
		dev_err(dev, "Failed to set up logger\n");
		goto pm_err;
	}

	error = cs40l26_asp_config(cs40l26);
	if (error) {
		dev_err(dev, "Failed to configure ASP\n");
		goto pm_err;
	}
	...
	if (cs40l26->revid == CS40L26_REVID_B2) {
		...
		if (error) {
			dev_err(dev, "Failed to issue OWT reset\n");
			goto pm_err;
		}
	}
	...
	if (cs40l26->fw_id != CS40L26_FW_CALIB_ID) {
		error = cl_dsp_get_reg(cs40l26->dsp, "COMPENSATION_ENABLE", CL_DSP_XM_UNPACKED_TYPE,
				CS40L26_VIBEGEN_ALGO_ID, &reg);
		if (error) {
			dev_err(dev, "Failed to get COMPENSATION_ENABLE register\n");
			goto pm_err;
		}
...
int cs40l26_fw_swap(struct cs40l26_private *cs40l26, const u32 id)
{
...
#ifdef CONFIG_CS40L26_SAMSUNG_FEATURE
	dev_info(cs40l26->dev, "%s: fw_id:%d, id%d\n", __func__, cs40l26->fw_id, id);
#endif
	int error;
	...
	if (error) {
#ifdef CONFIG_CS40L26_SAMSUNG_FEATURE
		dev_info(cs40l26->dev, "%s: Firmware Upload Failed\n", __func__);
#endif
...
int cs40l26_probe(struct cs40l26_private *cs40l26)
{
	...
	error = devm_mfd_add_devices(cs40l26->dev, PLATFORM_DEVID_NONE, cs40l26_devs,
