...
static int __maybe_unused arm_smmu_runtime_resume(struct device *dev)
{
	...
	if (of_device_is_compatible(dev->of_node, "qcom,adreno-smmu"))
		cx_gdsc_log("%s: name: %s, power_count: %d \n", __func__, dev_name(dev), smmu->pwr->power_count);
...
static int __maybe_unused arm_smmu_pm_resume(struct device *dev)
{
	...
	if (of_device_is_compatible(dev->of_node, "qcom,adreno-smmu"))
		cx_gdsc_log("%s: name: %s, disable_depth: %d, suspend: %d, usage: %d power_count: %d\n", __func__, dev_name(dev), dev->power.disable_depth, dev->power.runtime_status, atomic_read(&dev->power.usage_count), smmu->pwr->power_count);
