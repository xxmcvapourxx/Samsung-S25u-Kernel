			   ...
			   u32 dmic, u32 tx_mode, bool enable)
{
...
#if defined(CONFIG_SEC_AUDIO_DEBUG)
	sec_audio_debug_codec_mic_clk_enable(enable, dmic);
#endif
	...
	dev_info(component->dev, "%s: DMIC%d dmic_clk_cnt %d mic_pair %d\n",
	...
	if (ret < 0) {
		dev_err(component->dev, "%s: DMIC%d snd_soc_component_update_bits %d dmic_clk_cnt %d mic_pair %d\n",
			__func__, dmic, ret, *dmic_clk_cnt, mic_pair);
		sdp_info_print("%s: DMIC%d snd_soc_component_update_bits %d dmic_clk_cnt %d mic_pair %d\n",
			__func__, dmic, ret, *dmic_clk_cnt, mic_pair);
	} else {
		ret = 0;
	}
...
static ssize_t lpass_cdc_proc_read(struct file *filep, char __user *buf, size_t size, loff_t *ppos)
{
	...
	ret = regdump_read(priv->regmap, TX_START_OFFSET, TX_MAX_OFFSET, buf, size, ppos);
	return ret;
}
