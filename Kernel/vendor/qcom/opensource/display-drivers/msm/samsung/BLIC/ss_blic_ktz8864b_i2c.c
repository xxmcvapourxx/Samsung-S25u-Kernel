...
__visible_for_testing struct ss_blic_ktz8864b_info blic_pinfo;
...
int ss_blic_ktz8864b_control(bool enable)
{
	...
	if (enable) {
		size = ARRAY_SIZE(ktz8864b_en);
		data = ktz8864b_en;
	} else {
		size = ARRAY_SIZE(ktz8864b_dis);
		data = ktz8864b_dis;
	}
...
static void ss_blic_ktz8864b_remove(struct i2c_client *client)
{
	return;
}
