...
*/
#include "PA2_S6E3HAF_AMB666FM01_panel.h"
#include "PA2_S6E3HAF_AMB666FM01_mdnie.h"
...
static int ss_module_info_read(struct samsung_display_driver_data *vdd)
{
	...
	/* White Coordinates */
	/* Convert u8 4byte to char string 8byte */
	len += sprintf(temp, "%02x%02x%02x%02X\n", read_buf[0], read_buf[1], read_buf[2], read_buf[3]);
	vdd->w_coordinate= kzalloc(len, GFP_KERNEL);
	if (!vdd->w_coordinate) {
		LCD_ERR(vdd, "fail to kzalloc for w_coordinate\n");
	} else {
		vdd->w_coordinate_len = len;
		strlcat(vdd->w_coordinate, temp, vdd->w_coordinate_len);
		LCD_INFO(vdd, "w_coordinate:%s  len:%d\n", vdd->w_coordinate, vdd->w_coordinate_len);
	}
...
void PA2_S6E3HAF_AMB666FM01_WQHD_init(struct samsung_display_driver_data *vdd)
{
	...
	vdd->h_buf = PA2_S6E3HAF_AMB666FM01_PDF_DATA;
	vdd->h_size = sizeof(PA2_S6E3HAF_AMB666FM01_PDF_DATA);
