...
void dsi_phy_hw_v7_2_store_str(struct dsi_phy_hw *phy, u32 *val)
{
	...
	DSI_PHY_INFO(phy, "base : 0x%p, val : 0x%X\n", phy->base, *val);
...
u32 dsi_phy_hw_v7_2_show_str(struct dsi_phy_hw *phy)
{
	...
	hstx_str = DSI_R32(phy, DSIPHY_CMN_GLBL_HSTX_STR_CTRL_0);
	DSI_PHY_INFO(phy, "cur base : 0x%p, hstx_str : 0x%X (0x00 ~ 0xFF)\n", phy->base, hstx_str);
...
void dsi_phy_hw_v7_2_store_vreg(struct dsi_phy_hw *phy, u32 *val)
{
	...
	vreg_ctrl_0_str = *val;
	DSI_PHY_INFO(phy, "base : 0x%p, val:0x%X, vreg_ctrl_0:0x%X->0x%X\n",
...
u32 dsi_phy_hw_v7_2_show_vreg(struct dsi_phy_hw *phy)
{
	...
	DSI_PHY_INFO(phy, "cur base : 0x%p, vreg_ctrl_0 : 0x%02X \n", phy->base, val);
...
#if IS_ENABLED(CONFIG_DISPLAY_SAMSUNG)
	if (phy->version == DSI_PHY_VERSION_7_2) {
...
static void dsi_phy_hw_dphy_enable(struct dsi_phy_hw *phy, struct dsi_phy_cfg *cfg)
{
	...
	LCD_INFO_ONCE(vdd, "PHY_%d (0x%p), display_index:%d ver : %d, bit_clk : %ld\n",
...
/* SM8750 TODO:Compare with 8550 phy tune guide */
#if IS_ENABLED(CONFIG_DISPLAY_SAMSUNG)
...
void dsi_phy_hw_v7_2_dyn_refresh_pipe_delay(struct dsi_phy_hw *phy, struct dsi_dyn_clk_delay *delay)
{
	...
	if (delay->pll_reg_flush_delay) {
		...
		DSI_GEN_W32(phy->dyn_pll_base, DSI_DYN_REFRESH_PLL_REG_FLUSH_DELAY, delay->pll_reg_flush_delay);
		DSI_GEN_W32(phy->dyn_pll_base, DSI_DYN_REFRESH_PLL_REG_POST_FLUSH_DELAY, delay->pll_reg_post_flush_delay);
	}
