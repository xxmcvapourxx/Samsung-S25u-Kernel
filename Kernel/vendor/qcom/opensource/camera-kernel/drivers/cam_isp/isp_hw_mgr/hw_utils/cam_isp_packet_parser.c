			...
			if (new_head_entry && old_head_entry != new_head_entry) {
				...
				CAM_DBG(CAM_ISP,
					...
					out_map_entry->buffer_tracker->ion_fd, res->res_id);
                        ...
                        if (res->res_type == CAM_ISP_RESOURCE_SFE_OUT)
                            buf_info->prepare->sfe_en_out_res_mask |=
                                BIT_ULL(res->res_id - CAM_ISP_SFE_OUT_RES_BASE);
                        else
                            buf_info->prepare->ife_en_out_res_mask |=
                                BIT_ULL(res->res_id - CAM_ISP_IFE_OUT_RES_BASE);
            
                        CAM_DBG(CAM_ISP, "res type %d Res_idx = %d, mask =0x%llX acq mask = 0x%llx",
                            res->res_type, (res->res_id - CAM_ISP_IFE_OUT_RES_BASE),
                            buf_info->prepare->ife_en_out_res_mask,
                            buf_info->prepare->ife_acquire_wm_mask);			
		}
	}
