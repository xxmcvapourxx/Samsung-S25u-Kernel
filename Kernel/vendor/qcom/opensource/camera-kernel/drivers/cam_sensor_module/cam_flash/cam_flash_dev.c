...
#if defined(CONFIG_SAMSUNG_PMIC_FLASH)
		if (g_flash_ctrl && !g_flash_ctrl->sysfs_flash)
#endif
		cam_flash_off(fctrl);
		fctrl->flash_state = CAM_FLASH_STATE_ACQUIRE;
		break;
...
#if defined(CONFIG_SAMSUNG_PMIC_FLASH)
    fctrl->sysfs_flash = false;
	g_flash_ctrl = fctrl;
#endif
#if defined(CONFIG_SAMSUNG_PMIC_FLASH)
	if (msm_camera_pinctrl_init(&(fctrl->power_info.pinctrl_info), &pdev->dev) >= 0) {
		// make pin state to suspend
		rc = pinctrl_select_state(fctrl->power_info.pinctrl_info.pinctrl,
			fctrl->power_info.pinctrl_info.gpio_state_suspend);
		if (rc < 0) {
