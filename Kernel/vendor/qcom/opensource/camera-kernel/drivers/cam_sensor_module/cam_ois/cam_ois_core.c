...
static int cam_ois_thread_func(void *data)
{
	...
	CAM_INFO(CAM_OIS, "Enter: THREAD Started...");

	msg.u.node_msg.event_cause = 0xDEADBEEF;
	if (!data) {
		CAM_ERR(CAM_OIS, "Invalid Args");
		return -EINVAL;
	}
	...
	wait_event_freezable(o_ctrl->wait,
		o_ctrl->wakeup_condition || kthread_should_stop());

	CAM_INFO(CAM_OIS, "START to V4L2 Notify");

	while (true) {
		/*Lokesh: Just a Dummy code to send event to UMD Deamon*/
		usleep_range(20000, 30000);
		iterate--;
		if(cam_ois_notify_message(&(o_ctrl->v4l2_dev_str.sd),
			&msg,
			V4L_EVENT_CAM_OIS_FACTORY_EVENT,
			V4L_EVENT_CAM_OIS_EVENT)) {
			CAM_ERR(CAM_OIS,
				"Error in notifying Factory Event");
		}
		CAM_ERR(CAM_OIS, "lok: %d. Posting Event to UMD Deamon: 0x%x",
			iterate,  msg.u.node_msg.event_cause);
		msg.u.node_msg.event_cause--;
		if (iterate == 0)
			break;
	}
...
/**
 * cam_ois_handle_factory_cmd - Parse csl packet
 * @o_ctrl:     ctrl structure
 * @arg:        Camera control command argument
 *
 * Returns success or failure
 */
static int cam_ois_handle_factory_cmd(struct cam_ois_ctrl_t *o_ctrl, void *arg)
{
	int32_t                         rc = 0;
	...
	if (dev_config.test_status > 0) {
		CAM_INFO(CAM_OIS, "Send event");
		ois_handle_factory_test_result(&dev_config);
		o_ctrl->wakeup_condition = true;
		wake_up(&(o_ctrl->wait));
	}
#else
	if (dev_config.test_status == 0xDEADBEEF) {
		CAM_INFO(CAM_OIS, "lok: OIS Factory CMD START KTHREAD : %0x%x", dev_config.test_status);
		...
		if (IS_ERR(o_ctrl->ois_thread))
			CAM_ERR(CAM_OIS, "Unable to create OIS THREAD.......");
	...
	if (dev_config.test_status == 0xBEEF) {
		CAM_ERR(CAM_OIS, "Wake-up/unblock KTHREAD to send Notify Events\n");
		o_ctrl->wakeup_condition = true;
		wake_up(&(o_ctrl->wait));
	}

	CAM_INFO(CAM_OIS, "lok: process IOCTL EXIT: %0x%x", dev_config.test_status);
#endif
	return rc;
}
...
int cam_ois_driver_cmd(struct cam_ois_ctrl_t *o_ctrl, void *arg)
{
	...
	case CAM_CONFIG_DEV:
		...
		break;
	case CAM_FACTORY_TEST:
		rc = cam_ois_handle_factory_cmd(o_ctrl, arg);
		if (rc) {
			CAM_ERR(CAM_OIS, "Failed in ois pkt Parsing");
			goto release_mutex;
		}
