...
static int cam_icp_ctx_handle_buf_done_in_ready(void *context, void *done_evt_data)
{
	...
	buf_data = (struct cam_hw_done_event_data *)buf_done->buf_done_data;

	if (buf_data->evt_param == CAM_SYNC_ICP_EVENT_CMD_ERROR) {
		spin_lock(&ctx->lock);
		...
		if (req)
			cam_context_dump_patch_info_to_hw(ctx, req);
	}
